...
namespace ApiGetwayMock
{
    public class LoyaltyProgramClient
    {
        ...
        {
            var userResource = $"api/users/{userId}";
        ...
        private async Task<HttpResponseMessage> DoRegisterUser(LoyaltyProgramUser newUser)
        {
            ...
            {
                httpClient.BaseAddress = new Uri($"http://{this.hostName}");
                var response = await httpClient.PostAsync("api/users", new StringContent(JsonConvert.SerializeObject(newUser), Encoding.UTF8, "application/json"));
                ThrowOnTransientFailure(response);
                return response;
            }
        }
        ...
        private async Task<HttpResponseMessage> DoUpdateUser(LoyaltyProgramUser user)
        {
            ...
            {
                httpClient.BaseAddress = new Uri($"http://{this.hostName}");
                var response = await httpClient.PutAsync($"api/users/{user.Id}", new StringContent(JsonConvert.SerializeObject(user), Encoding.UTF8, "application/json"));
                ThrowOnTransientFailure(response);
                return response;
            }
        }
